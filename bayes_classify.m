...
if isequal(mu_a,mu_b) && isequal(v_a,v_b)
    ...
    if isempty(parser.Results.custom_bd_fns) % if calculated or custom coefficients
        [acc_norm_a,bd_pts_norm_a]=integrate_normal(mu_a,v_a,'bd_coeffs',bd_coeffs_norm_a,'p_prior',p_a,'plot_color','blue');
        [acc_norm_b,bd_pts_norm_b]=integrate_normal(mu_b,v_b,'bd_coeffs',bd_coeffs_norm_b,'p_prior',p_b,'plot_color','red');
    else % if arbitrary boundary functions
        [acc_norm_a,bd_pts_norm_a]=integrate_normal(mu_a,v_a,'bd_fn',bd_fn_a,'p_prior',p_a,'plot_color','blue');
        [acc_norm_b,bd_pts_norm_b]=integrate_normal(mu_b,v_b,'bd_fn',bd_fn_b,'p_prior',p_b,'plot_color','red');
    end
...
results.errmat_norm=[acc_norm_a, 1-acc_norm_a; 1-acc_norm_b, acc_norm_b];
results.err_norm=1-acc_norm;

% d'
if optimal_case
    d_norm=2*norminv(acc_norm);
