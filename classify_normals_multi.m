function results=classify_normals_multi(normals,varargin)
% Classification between two distributions.
%
% How to use this command:
% See github readme at https://github.com/abhranildas/classify
%
% Credits:
%   Abhranil Das <abhranil.das@utexas.edu>
%	Wilson S Geisler
%	Center for Perceptual Systems, University of Texas at Austin
% If you use this code, please cite:
%   A new method to compute classification error
%   https://jov.arvojournals.org/article.aspx?articleid=2750251

% parse inputs
n_normals=length(normals);
parser = inputParser;
addRequired(parser,'normals',@isstruct);
addParameter(parser,'priors',ones(1,n_normals)/n_normals,@(x) isnumeric(x) && all(x > 0) && all(x < 1));
addParameter(parser,'vals',eye(n_normals),@(x) isnumeric(x) && ismatrix(x));
addParameter(parser,'type','norm',@(s) strcmp(s,'norm') || strcmp(s,'samp'));
addParameter(parser,'bPlot',true,@islogical);

parse(parser,normals,varargin{:});
vals=parser.Results.vals;
bPlot=parser.Results.bPlot;

if strcmp(parser.Results.type,'norm')
    dim=length(normals(1).mu);
    mus=nan(dim,n_normals);
    vs=nan(dim,dim,n_normals);
    for i=1:n_normals
        mus(:,i)=normals(i).mu;
        vs(:,:,i)=normals(i).v;
    end
else
    dim=size(normals(1).sample,2);
    mus=nan(dim,n_normals);
    vs=nan(dim,dim,n_normals);
    for i=1:n_normals
        mus(:,i)=mean(normals(i).sample);
        vs(:,:,i)=cov(normals(i).sample);
    end
end

if dim>3
    error('Multi-class classification can only handle 3 or fewer dimensions.')
end

% if sample input and priors are not specified,
if strcmp(parser.Results.type,'samp') && any(strcmp(parser.UsingDefaults,'priors'))
    % set priors according to input sample sizes
    priors=arrayfun(@(x) length(x.sample),normals);
    priors=priors/sum(priors);
else
    priors=parser.Results.priors;
end

if bPlot, figure; hold on; end

norm_bd_pts=cell(n_normals,1);
norm_err_mat=nan(n_normals);
norm_errs=nan(n_normals,1);
% compute accuracy and boundary for each normal, and combine
for i=1:n_normals % integrate each normal
    for j=1:n_normals % within the boundary of each normal
        [p,pc,bd_pts]=integrate_normal(mus(:,i),vs(:,:,i),...
            @(n,mu,v) opt_class_multi(n,mus,vs,j,'priors',priors,'vals',vals,'mu',mu,'v',v),...
            'reg_type','ray_scan','bPlot',false);
        fprintf('Integrating normal %d in region %d\n',[i j])        
        norm_err_mat(i,j)=p;
        if j==i
            norm_errs(i)=pc;
        end
        norm_bd_pts{j}=[norm_bd_pts{j},bd_pts];
    end
end

% trim to unique boundary points
for i=1:n_normals
    norm_bd_pts{i}=uniquetol(norm_bd_pts{i}',1e-12,'Byrows',true,'Datascale',1)';
end

norm_err=dot(priors,norm_errs);

results.norm_bd_pts=norm_bd_pts;
results.norm_err_mat=norm_err_mat;
results.norm_err=norm_err;

if ~isequal(vals,eye(n_normals)) % if outcome values are supplied
    results.norm_val_mat=results.norm_err_mat.*vals; % conditional expected values
    results.norm_val=sum(sum(results.norm_val_mat.*priors'));
end

%% Plot data
if bPlot
    hold on
    colors=colororder;
    title(sprintf("error = %g",norm_err),'interpreter','latex') % plot title
    
    % Colormap of the different regions
%     fcontour(@(x,y) multifun(x,y,normals,1),'levellist',0.5:1:4.5,'fill','on','linecolor','none');
%     caxis([.5 4.5]);
%     colormap((colors(1:4,:)+[1 1 1])/2)

    % plot samples
    for i=1:n_normals
        if strcmp(parser.Results.type,'samp')
            plot_sample(normals(i).sample,priors(1),colors(i,:))
        end
    end
    % plot normals and boundaries
    for i=1:n_normals
        plot_normal(mus(:,i),vs(:,:,i),priors(i),colors(i,:))
        hold on
        plot_boundary(@(n,mu,v) opt_class_multi(n,mus,vs,i,'mu',mu,'v',v,'priors',priors,'vals',vals),dim,'mu',mus(:,i),'reg_type','ray_scan')
    end
    hold off
end