...
parser = inputParser;
addRequired(parser,'normals',@(x) isstruct(x));
...
dim=length(normals(1).mu); % dimension
...
for i=1:n_normals % integrate each normal
    for j=1:n_normals % within the boundary of each normal
        [p,pc,bd_pts]=integrate_normal(normals(i).mu,normals(i).v,'bd_fn',@(n) opt_bd_multi(n,normals(circshift(1:n_normals,-(j-1)))));
        [i j p]
    end
end
%normals=normals(circshift(1:n_normals,-1)); % cycle through the normals
