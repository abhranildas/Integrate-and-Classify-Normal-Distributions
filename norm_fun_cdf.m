function p=norm_fun_cdf(x,mu,v,fun,varargin)
    ...
    if isa(fun,'function_handle')
        if strcmpi(side,'lower')
    		p=arrayfun(@(fun_level) integrate_normal(mu,v,fun,'dom_type','fun','fun_level',fun_level,'side','complement',varargin{:},'plotmode',false), x);
        elseif strcmpi(side,'upper')
            p=arrayfun(@(fun_level) integrate_normal(mu,v,fun,'dom_type','fun','fun_level',fun_level,varargin{:},'plotmode',false), x);
        end
    elseif isstruct(fun)
        [w,k,lambda,m,s]=norm_quad_to_gx2_params(mu,v,fun);
        p=gx2cdf(x,w,k,lambda,m,s,varargin{:});
    end
end
